...
def generate_synthetic_logs(n_rows=5000, corr_strength=0.5, filename="log.csv"):
    ...
    statuses, bytes_sent, latency_ms = [], [], []
...
def main():
    ...
    os.chdir("logs")
    datasets = [("log_low.csv", 0.1, 2000), ("log_medium.csv", 0.5, 10000), ("log_high.csv", 0.9, 30000)]
    results = []
    ...
    for _, row in res_df.iterrows():
        print(f"{row['Dataset']:15} | Rows={int(row['Rows']):5d} | Corr={row['Corr']:.1f} | Orig={sizeof_fmt(row['Original'])} | GZIP={sizeof_fmt(row['GZIP'])} | Chow–Liu total={sizeof_fmt(row['ChowLiu_Total'])} (data={sizeof_fmt(row['ChowLiu_Data'])}, model={sizeof_fmt(row['ChowLiu_Model'])})")
