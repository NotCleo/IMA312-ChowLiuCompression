...
2) Learns Chow–Liu dependency trees from each
...
def generate_synthetic_logs(n_rows=5000, corr_strength=0.5, filename="log.csv"):
    ...
    for i in range(n_rows):
        ...
        error_prob = 0.05 + corr_strength * (0.2 if m in ['POST', 'PUT'] else 0) + (0.1 if 0 <= h < 6 else 0)
        if np.random.rand() < error_prob:
            status = np.random.choice([404, 500, 403], p=[0.5, 0.3, 0.2])
        else:
            status = 200

        # Correlate bytes_sent with status and method
        mean_bytes = 5000 if status == 200 else 500
        mean_bytes += 2000 if m == 'POST' else -1000 if m == 'DELETE' else 0
        bytes_val = max(0, int(np.random.normal(mean_bytes, 1000)))

        # Correlate latency with bytes_sent and hour (e.g., slower at peak hours)
        mean_latency = 50 + (bytes_val / 100) + corr_strength * (50 if 12 <= h < 18 else 0)
        lat_val = max(10, int(np.random.normal(mean_latency, 30)))
...
def learn_chow_liu_tree(df):
    """Learn Chow–Liu dependency tree via mutual information."""
...
def estimate_chow_liu_compression(df, T):
    ...
    # Compute total_MI
    total_MI = sum(data['weight'] for _, _, data in T.edges(data=True))
...
def visualize_tree(T, filename):
    ...
    cbar.set_label("Mutual Information (bits)", rotation=270, labelpad=15)
    ax.set_title("Chow–Liu Dependency Tree", fontsize=14)
    plt.tight_layout()
...
def main():
    ...
    datasets = [
        ("log_low.csv", 0.1, 2000),   # Low correlation, small size
        ("log_medium.csv", 0.5, 10000),  # Medium correlation, medium size
        ("log_high.csv", 0.9, 30000),  # High correlation, large size
    ]
    ...
    for _, row in res_df.iterrows():
        print(f"{row['Dataset']:15} | Rows={int(row['Rows']):5d} | Corr={row['Corr']:.1f} | "
    ...
    plt.title("Compression Comparison: Original vs GZIP vs Chow–Liu")
